...
- [x] FCL Pipeline Explorer 已纳入银行专业作品区与相关文章，三语切换完整覆盖。
...
- [x] 首页已加入左侧可拖动陪伴角色：五种透明表情、全页面持续显示、双端位置记忆、控件避让和三语无障碍均已接入。
- [x] 首页已删除重复的个人产品场景区，银行金融与数据产品作品紧接个人产品，并在导航提供直达入口。
- [x] 已新增中文零基础 AI 建站单文件教程，覆盖 GitHub 账号、本地项目目录、AI 代办提示词、GitHub Pages、阿里云域名、DNS、HTTPS 与排错。
...
- `docs/AI_WEBSITE_BUILD_GUIDE.html`：可离线分享的中文 AI 建站实战教程；截图、流程图、样式与交互均内联，面向零基础用户并优先通过可复制提示词让 AI 完成技术步骤。
...
- 陪伴角色在首页全程跟随视口，不因进入创作者、文章或联系区域而隐藏；拖动位置按桌面/移动端分别保存在 localStorage，遮挡控件时自动缩小淡化。
- AI 建站教程保持为独立单文件，不接入当前首页构建；更新 GitHub、阿里云、DNS 或备案步骤时应重新核对官方文档，并继续区分“本人安全操作”与“AI 可代办技术任务”。

## 变更记录

| 日期 | 变更内容 |
|------|----------|
| 2026-08-20 | 在教程本地项目初始化章节增加“记录重要开发过程”推荐提示词，约定将关键需求和决策保存到 `docs/development/` 并保留决策演变；why：让跨会话、跨工具继续开发时可以复用项目上下文，而不记录普通聊天造成文档泛滥 |
| 2026-08-20 | 在教程工具表和安装说明中补充 CodeBuddy 官方下载地址及替代 Codex 的提示词替换说明；why：让使用 CodeBuddy 的读者能从官方入口开始，同时保持 Codex 为默认主线 |
| 2026-08-15 | 重写教程的本地文件夹章节，明确项目可建在任意合适路径，并推荐“项目集合文件夹 / 具体项目文件夹”两层结构，同时把命令行创建方法标为非必要；why：避免小白误以为必须使用 Finder 或固定的 Projects 路径，也防止把包含多个项目的上级目录交给 Codex |
| 2026-08-15 | 为教程中的 Git 命令了解和 DNS 命令验证添加“非必要”标签，并将 DNS 检查改为故障时才展开；why：避免小白把可跳过的理解与排错内容误认为上线必经步骤，也说明 DNS 常在几分钟内生效而 24 小时只是传播上限 |
| 2026-08-15 | 在教程首屏新增 GitHub 最新版查看与直接下载入口，并说明保存和离线打开方法；why：让收到旧副本的小白也能立即找到 main 分支中的最新教程 |
| 2026-08-15 | 在教程 DNS 章节补充 A 记录作用、GitHub Pages 四个官方 IPv4 入口的原因，以及根域名与子域名的配置分流；why：避免小白把只适用于根域名的 A 记录误用到 pagevoice 等 CNAME 子域名 |
| 2026-08-15 | 删除教程需求访谈提示词中“不限制问题总数但不要重复询问”的附加要求，只保留有疑问就继续提问；why：避免对 Codex 的必要澄清方式增加额外限制 |
| 2026-08-15 | 取消教程需求访谈的提问数量上限，并把依赖安装、开发预览、构建、Git 提交与推送改为 Codex 直接执行的提示词流程；why：零基础用户不应被要求理解终端或手工输入可由编程助手安全完成的命令 |
| 2026-08-15 | 将零基础建站教程的默认开发工具统一为 Codex，移除 Claude Code 的安装与版本检查，并补充 WorkBuddy 等替代工具的名称替换说明；why：让提示词与真实建站流程一致，减少小白同时理解多个工具的负担 |
| 2026-08-15 | 从零基础 AI 建站教程中移除 GitHub 2FA 专门章节及恢复码操作，仅保留基础密码与验证码安全提醒；why：降低初学者进入建站主线前的额外负担 |
| 2026-08-15 | 新增可离线分享的中文 AI 建站零基础实战教程，加入 GitHub 注册、本地项目文件夹、AI 优先代办提示词、GitHub Pages、阿里云域名与 DNS/HTTPS 全流程；why：让朋友可从无账号、无项目目录开始，按明确的人机分工完成个人网站上线 |
| 2026-08-04 | 删除重复的个人产品场景区，将银行金融与数据产品作品提前并增加导航直达入口；why：减少首页重复，同时让招聘方更快发现 FCL、ALM 与 FTP 专业作品 |
| 2026-08-03 | 将首页陪伴角色改为从页首到页尾持续显示，并保留拖动位置记忆与控件避让；why：让网站小精灵在浏览创作者、文章和联系区域时也始终陪伴访客 |
